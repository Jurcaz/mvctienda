...
<div class="card p-4 bg-light">
    <?php $verify = false; $subtotal = 0; $send = 0; $discount = 0; $user_id = $dataView['user_id'] ?>
        ...
        <table class="table table-stripped" style="width: 100%">
            ...
            <?php foreach($dataView['data'] as $key => $value): ?>
                <tr>
                    ...
                    <td>&nbsp;</td>
                    <td class="text-right"><a href="<?= ROOT ?>cart/delete/<?= $value->product ?>/<?= $dataView['user_id'] ?>" class="btn btn-danger">Borrar</a></td>
                </tr>
                <?php $subtotal += $value->price * $value->quantity; $discount += $value->discount; $send += $value->send ?>
            <?php endforeach; ?>
            <?php $total = $subtotal - $discount + $send ?>
            <input type="hidden" name="rows" value="<?= count($dataView['data']) ?>">
            <input type="hidden" name="user_id" value="<?= $dataView['user_id'] ?>">
        </table>
        <hr>
